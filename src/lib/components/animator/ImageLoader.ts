...
export class ImageLoader {
  ...
  `;
	...
	static readonly blueColor = '#2E3192'; // HEX_BLUE as specified
	static readonly redColor = '#ED1C24'; // HEX_RED as specified
	...
	static loadImages(): Promise<{
		...
		return new Promise((resolve, reject) => {
			...
			const gridSvgDataUrl =
				'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(this.gridSvgString);
			const blueStaffSvgDataUrl =
				'data:image/svg+xml;charset=utf-8,' +
				encodeURIComponent(this.staffBaseSvgString(this.blueColor));
			const redStaffSvgDataUrl =
				'data:image/svg+xml;charset=utf-8,' +
				encodeURIComponent(this.staffBaseSvgString(this.redColor));
