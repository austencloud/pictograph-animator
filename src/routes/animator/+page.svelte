<script lang="ts">
	...
	import SequenceImportExport from '../../lib/components/animator/SequenceImportExport.svelte';
	...
	onMount(async () => {
		try {
			...
			// Update the sequences and selected sequence after loading
	...
	// Handle imported sequence
	function handleImport(event: CustomEvent<{ sequence: SequenceData }>) {
		console.log('Imported sequence:', event.detail.sequence);
		selectedSequence = event.detail.sequence;

		// Also update the registry with this sequence
		// This allows us to save it for later use
		if (selectedSequence[0].word) {
			const id = selectedSequence[0].word.toLowerCase().replace(/\s+/g, '_');
			sequenceRegistry[id] = selectedSequence;
		}
	}
</script>
	...
	{#if !isLoading}
		...
		<div class="settings">
			...
			<div class="setting">
				...
				<input id="speed" type="range" min="0.1" max="3.0" step="0.1" bind:value={animationSpeed} />
			</div>
		</div>
		...
		<div class="sequence-info">
			...
			<ul>
				...
				<li>
					<strong>Dash:</strong> Staff maintains orientation but may have specific end orientation
				</li>
			</ul>
			...
			<SequenceImportExport sequence={selectedSequence} on:import={handleImport} />
		</div>
